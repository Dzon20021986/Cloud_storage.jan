...
public class ServerMainHandler extends ChannelInboundHandlerAdapter {
    ...
    @Override
    public void channelActive(ChannelHandlerContext ctx) {  // канал активен
    ...
    public void channelRead(ChannelHandlerContext ctx, Object message) throws Exception {  // чтение канала
        try {
            ...
            if (message instanceof FileMessage) {
                ...
                Files.write(Paths.get("server_storage/" + fileMessage.getFilename()), fileMessage.getData(), StandardOpenOption.CREATE); //StandardOpenOption.CREATE всегда создаёт/перезаписывает новые объекты
                refreshServerListView(ctx);
            }
    ...
    @Override
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {  // пойманное исключение
        cause.printStackTrace();
        ctx.close();
    }

    private void refreshServerListView(ChannelHandlerContext ctx) {  // обновление списка
        try {
